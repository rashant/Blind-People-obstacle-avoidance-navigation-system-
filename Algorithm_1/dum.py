    ...
    for i in range(255,0,-40):
        ...
        for c in cnts:
            ...
            if(area>2000):
                ...
                cv2.putText(img,f"Angle:- {angle}",(int(cols/2)+5,cols), cv2.FONT_HERSHEY_COMPLEX_SMALL,0.5,(0,255,0),1,cv2.LINE_AA)
                ...
                # cv2.imshow("image",ximg)
                # cv2.imshow("crop",crop_img)
                # cv2.imshow("Path",line_frame)
                print(img.shape,xmask.shape)
